<template>
  <div class="subject-view has-header">
    <banner :title="typeClassifyText"></banner>
      ...
      <div class="subject-card">
        ...
        <div class="subject-info">
          ...
          <div class="left" v-if="subject.rating">
            ...
            <template v-if="genres && subjectMeta">
              <p class="meta">{{subjectMeta}}</p>
              <a href="#" class="open-app" v-text="dataApp"></a>
            </template>
            <template v-if="subject.author && subjectMeta">
              <a href="#" class="open-app buy">在豆瓣购买</a>
            </template>
          </div>
        </div>
        <div v-if="subject.author" class="vendors-link">
          <a class="link">在哪儿买这本书？ <span class="info">
              豆瓣阅读电子书 66.00元起
            </span></a>
        </div>
        <marking>
...
<script type="text/ecmascript-6">
  ...
  export default{
    ...
    computed: {
      ...
      typeClassifyText: function () {
        return this.typeClassify === 'movie' ? "聊聊你的观影感受" : "聊聊对此书的看法"
      },
      dataApp: function () {
        return this.typeClassify === 'movie' ? "用App查看影人资料" : "用App查看作者资料"
      }
    },
    methods: {

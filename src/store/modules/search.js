...
const actions = {
  ...
  query({commit}, payload){
    ...
    // Getting book
    request
      .get('https://api.douban.com/v2/book/search?q=' + payload.queryStr + '&count=3')
      .use(jsonp)
      .end((err, res) => {
        if (!err) {
          commit({
    ...
    // Getting music
    request
      .get('https://api.douban.com/v2/music/search?q=' + payload.queryStr + '&count=3')
      .use(jsonp)
      .end((err, res) => {
        if (!err) {
          commit({

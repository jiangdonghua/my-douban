  ...
  getMovie(state, payload){
    switch (payload.tag) {
      case 'hotMovies':
        ...
        break
      case 'soonMovies':
        state.soonMovies = payload.res
        break
      case 'topMovies':
        state.topMovies = payload.res
        break
      // case 'weeklyMovies':
      //   state.weeklyMovies = payload.res
      //   break
      case 'northMovies':
        state.northMovies = payload.res
        break
      // case 'newMovies':
...
const actions = {
  /**
   * Getting movies
   * q: in_theaters, coming_soon, top250,weekly,us_box,new_movies
   * count: 8
   */
  getMovie({commit}){
    request
    ...
    // request
    //   .get('https://api.douban.com/v2/movie/weekly?&count=8')
    //   .use(jsonp)
    //   .end((err, res) => {
    //     if (!err) {
    //       commit({
    ...
    //   })
    request
      .get('https://api.douban.com/v2/movie/us_box?&count=8')
      .use(jsonp)
      .end((err, res) => {
        if (!err) {
          commit({
    ...
    // request
    //   .get('https://api.douban.com/v2/movie/new_movies?&count=8')
    //   .use(jsonp)
    //   .end((err, res) => {
    //     if (!err) {
    //       commit({
